...
export const updateCompany = async (req:Request, res:Response) => {
    ...
    try {
        const result = await pool.query(
        `UPDATE companies SET company_name=$1, sector=$2,email=$3,phone=$4,address=$5,location=$6,website=$7, contact_name=$8, contact_email=$9, contact_phone=$10
...
export const deleteCompany = async (req:Request, res:Response) => {
    ...
    try {
        ...
        if (result.rowCount === 0) {
            return res.status(404).json({success:false, message: "Cannot find the company in your records"})
        }
